...
```text
Use Serena to navigate symbols, use CocoIndex to search for relevant implementation areas, and use $caveman lite for a concise answer. Start by checking .agents/INDEX.md.
```
...
```text
Use the Serena MCP server for semantic code navigation and symbolic edits. Activate project /absolute/path/to/cpp-ai-template and read Serena's initial instructions before broad exploration. Use the CocoIndex MCP server for natural-language codebase search. Load repo skills from .agents/skills when a task matches their SKILL.md descriptions.
```
...
rg "search term" README.md AGENTS.md .agents .codex cmake include src tests examples
...
```text
Before editing, read README.md, .agents/INDEX.md, and .agents/CONVENTIONS.md. Activate Serena for this project, read its initial instructions, use Serena for symbol navigation, use CocoIndex for broad code search, and keep the answer concise.
```
